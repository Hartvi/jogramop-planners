...
// A simple structure to hold JPlusRbtPlanner instance
typedef struct
{
    PyObject_HEAD
        Burs::JPlusRbtPlanner *planner;
} URDFPlannerObject;
...
static PyObject *URDFPlanner_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    if (self != NULL)
    {
        ...
        self->planner = new Burs::JPlusRbtPlanner(std::string(urdf_file), max_iters, d_crit, delta_q, epsilon_q, num_spikes);
    }
...
static PyObject *URDFPlanner_GetNrOfJoints(URDFPlannerObject *self)
{
    if (!self->planner)
    {
        PyErr_SetString(PyExc_TypeError, "Invalid JPlusRbtPlanner object");
        return NULL;
    }
...
PyMODINIT_FUNC PyInit_Burs(void)
{
    ...
    if (PyModule_AddObject(m, "JPlusRbtPlanner", (PyObject *)&URDFPlannerType) < 0)
    {
