...
namespace Burs
{
    ...
    JRbtPlanner::JRbtBasicComplete(const VectorXd &q_start, JPlusRbtParameters &plan_params, PlanningResult &plan_result)
    {
        ...
        for (unsigned int k = 0; k < plan_params.max_iters; ++k)
        {
            ...
            if (too_close)
            {
                int step_result = this->RRTStepInQ(tree, nearest_idx, Qe_states[0], plan_params.epsilon_q, plan_params.collision_resolution, true, false);
                if (step_result >= 0)
                {
            ...
            if (rand_num < plan_params.probability_to_steer_to_target)
            {
                ...
                AlgorithmState state = this->ExtendToGoalRRT(tree, plan_params, false);
    ...
    JRbtPlanner::JRbtBasic(const VectorXd &q_start, JPlusRbtParameters &plan_params, PlanningResult &plan_result)
    {
        ...
        for (unsigned int k = 0; k < plan_params.max_iters; ++k)
        {
            ...
            // Create only one state if too close
            std::vector<RS> Qe_states = this->NewStates((too_close ? Qe.col(0) : Qe), true);
            ...
            {
                double distance_to_move = d_closest;
                std::vector<RS> endpoints = this->GetEndpointsCollisionCheck(*near_state, Qe_states, distance_to_move);
    ...
    std::optional<std::vector<VectorXd>>
    JRbtPlanner::JRbtBasicOriginal(const VectorXd &q_start, JPlusRbtParameters &plan_params, PlanningResult &plan_result)
    {
        std::cout << "RUNNING JRBT ORIGINAL\n";
        if (plan_params.target_poses.size() < 1)
        {
            throw std::runtime_error("Target poses has length 0!");
        }
        ...
        for (unsigned int k = 0; k < plan_params.max_iters; ++k)
        {
            ...
            {
                double distance_to_move = d_closest;
                std::vector<RS> endpoints = this->GetEndpointsCollisionCheckOriginal(*near_state, Qe_states, distance_to_move);
    ...
    JRbtPlanner::ExtendToGoalRbt(std::shared_ptr<BurTree> tree, JPlusRbtParameters &plan_params) const
    {
        ...
        */
        ...
        do
        {
            ...
            std::vector<RS> target_states = this->NewStates(target_configs, true);
