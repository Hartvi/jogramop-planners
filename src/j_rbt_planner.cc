...
namespace Burs
{
    ...
    // {
    //     assert(plan_params.distanceEstimateType == DistanceEstimateType::JacPosRot);
    ...
    //         if (too_close)
    //         {
    //             int step_result = this->RRTStepInQ(tree, nearest_idx, Qe_states[0], plan_params.epsilon_q, plan_params.collision_resolution, plan_params.distanceEstimateType);
    //             if (step_result >= 0)
    //             {
        ...
        for (unsigned int k = 0; k < plan_params.max_iters; ++k)
        {
            ...
            else // REGULAR BUR
            {
                ...
                for (unsigned int i = 0; i < endpoints.size(); ++i)
                {
                    ...
                    //     std::cout << "ENDPOINT SHOULD NOT BE COLLIDING\n";
                    //     num_bad_crashes++;
                    //     throw std::runtime_error("RBT STEP COLLIDING");
                    // }
                    // else
                    // {
                    getTime(&tt1);
    ...
    JRbtPlanner::ExtendToGoalRbt(std::shared_ptr<BurTree> tree, JPlusRbtParameters &plan_params) const
    {
        ...
        */
        ...
        do
        {
            ...
            std::vector<RS> target_states = this->NewStates(target_configs, DistanceEstimateType::None);
