...
namespace Burs
{
    ...
    // AlgorithmState
    // JPlusRbtPlanner::ExtendToGoal(std::shared_ptr<BurTree> t_start, const Eigen::VectorXd &q_near, const JPlusRbtParameters &planner_parameters, PlanningResult &planning_result, VectorXd &last_q)
    // {
    ...
    //     while (current_distance < planner_parameters.d_crit)
    //     {
    //         // update current configuration to a random one
    //         KDL::JntArray q_kdl(q_near.size());
    //         // q_kdl.data = q_near;
    ...
    //         {
    //             int nearest_index = t_start->Nearest(q_new.data());
    //             t_start->AddNode(nearest_index, q_new);
