...
class Circle(Shape):
    ...
    def plot(self, ax: plt.axes, figsize, **kwargs):
        ...
        ax.scatter(self.position[0], self.position[1],
                   marker="o", s=points_radius**2, **kwargs)


class LineRobot:
    def __init__(self, segment_lengths: np.ndarray) -> None:
        self.num_segments: int = len(segment_lengths)
        self.segment_lengths: np.ndarray = np.concatenate(
            [[0], segment_lengths])
        self.positions: np.ndarray = np.zeros((2, self.segment_lengths.size))
        self.causality_mask: np.ndarray = np.zeros(
            (len(segment_lengths), len(segment_lengths)))
    ...
    def forward(self, q: np.ndarray) -> np.ndarray:
        ...
        assert len(q) == (len(self.segment_lengths) - 1), "q: " + \
            str(len(q)) + " segments: " + str(len(self.segment_lengths))
        final_positions: np.ndarray = np.concatenate([[np.cumsum(self.segment_lengths)], [
                                                     np.zeros(len(self.segment_lengths))]], axis=0).T
        ...
        for i in range(len(final_positions) - 1, 0, -1):
            ...
            for k in range(i, len(final_positions)):
                # R(i)*(p(i) - p(i-1)) + p(i-1)
                final_positions[k] = get_rot(
                    q[i - 1]) @ (final_positions[k] - final_positions[i - 1]) + final_positions[i - 1]
    ...
    def scatter_distance_configs(self, q: np.ndarray, max_dist, ax: plt.axes, color) -> list[list[float]]:
        ...
        for i in range(rotation_resolution_edge):
            ...
            self.scatter_config(tmp_q, ax, color=color, s=5)
    ...
    def scatter_distance_configs_extended(self, q: np.ndarray, min_idx: int, max_dists: np.ndarray, ax: plt.axes, color) -> list[list[float]]:
        ...
        for i in range(rotation_resolution_edge):
            ...
            for k in range(min_idx+1, q.size):
                ...
                tmp_q = self.extend_in_direction(
                    tmp_q, q_r, max_dists[k] - max_dists[k-1], epsilon_q)
        ...
        if isinstance(shape, Circle):
            ...
            for i in range(1, len(self.positions)):
                ...
                if np.linalg.norm(c_on_b) < np.linalg.norm(b) and np.dot(b, c) > 0:
                    ...
                    tmp_dist = min(np.linalg.norm(
                        c), np.linalg.norm(c - b)) - shape.radius
...
if __name__ == "__main__":
    ...
    if plot_thing == 0:
        ...
        for i in range(0, num_burs):
            ...
            env.plot_extended_bur(
                rand_config, (0.5, rand_color[0], rand_color[1]))
