...
/** box counting algorithm for estimating dimension of the point set, resp. a curve defined by the waypoints */
double boxCounting(const std::vector< std::vector<double> > &waypoints, const double resolution) {
    if (waypoints.size() == 0) {
        WDEBUG("cannot computed box counting dimension from zero points ");
        return -1;
    }

    const int dimension = waypoints[0].size();


    vector<int> coordinates(dimension,0);
    vector<double> minvalues( waypoints[0] );
    vector<double> maxvalues( waypoints[0] );

    for(int i=0;i<(int)waypoints.size();i++) {
        for(int d = 0; d < dimension; d++) {
            if (waypoints[i][d] < minvalues[d]) {
                minvalues[d] = waypoints[i][d];
            }
            if (waypoints[i][d] > maxvalues[d]) {
                maxvalues[d] = waypoints[i][d];
            }
        }
    }


    std::map< vector<int>, int > grid;
    for(int i=0;i<(int)waypoints.size()-1;i++) {
    //    WDEBUG("waypoint[ " << i << "=" << printString(waypoints[i]));
      //  WDEBUG("waypoint[ " << (i+1) << "=" << printString(waypoints[i+1]));

        double dist = 0;
        for(int j = 0; j < dimension; j++) {
            const double diff = waypoints[i][j] - waypoints[i+1][j];
            dist += diff*diff;
        }
        dist = sqrt(dist);
        const int num = (int)lround(dist / resolution)+ 1;
        //WDEBUG("dist = " << dist);
        //WDEBUG("Conecting " << i << " and " << (i+1) << " with " << num << " intermediate ");

        vector< vector<double> > tmp;
        approximateStates(waypoints[i], waypoints[i+1], num, tmp);
    
        for(int t = 0; t <(int)tmp.size();t++) {
            for(int di = 0; di < dimension; di++) {
                const int idx = (int)( (tmp[t][di] - minvalues[di]) / resolution );
                coordinates[di] = idx;
            }
...
//    WDEBUG("Box counting at " << resolution << " occupied " << grid.size() << " cells ");
    return log(1.0*grid.size()) / log(1./resolution);
