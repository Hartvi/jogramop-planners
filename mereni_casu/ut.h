...
template<typename T>
std::string printString(T **array, const int rows, const int cols, const std::string delim = std::string(" "),
	   const std::string endDelim = std::string()) {
...
template<typename ST>
void approximateStates(const ST &s1, const ST &s2, const int num, const int maxResult, std::vector<ST> &result) {

	ST tmp(s1);
    
    result.clear();
    result.reserve(maxResult+1);
